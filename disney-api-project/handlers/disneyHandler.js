import { fetchCharacter, fetchOneCharacter } from '../connectors/disneyConnector.js';
...
export const getDisneyCharacter = async (req,res) => {
    ...
    try {
        ...
        res.status(500).json({ message: `Failed to fetch character for category ${characterId}`, error });
...
export const checkForCharacterByName = async (characterId, characterName) => {
    try {
      ...
      if (character.name && character.name.toLowerCase() === characterName.toLowerCase()) {
        return {
          exists: true,
